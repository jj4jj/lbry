...
class Daemon(AuthJSONRPCServer):
    ...
    def jsonrpc_blob_availability(self, blob_hash, search_timeout=None, blob_timeout=None):
        ...
        Returns:
            (dict) {
                ...
                "hosts": {
                    "<ip>:<port>": <bool, true if blob can be downloaded from host>
                }
            }
        """
        ...
        try:
            ...
            for peer_info in peer_infos:
                d = self.session.single_peer_downloader.download_temp_blob_from_peer(**peer_info)
                dl.append(d)
                dl_peers.append("%s:%i" % (peer_info['peer'].host, peer_info['peer'].port))
            for dl_peer, (success, download_result) in zip(dl_peers,
                                                           (yield defer.DeferredList(dl))):
                if success:
                    result[dl_peer] = download_result
            is_available = any(result.itervalues())
        except Exception as err:
    ...
    def jsonrpc_stream_availability(self, uri, search_timeout=None, blob_timeout=None):
        ...
        head_blob_hash = None
        try:
            sd_blob = yield self.jsonrpc_blob_get(sd_hash, timeout=blob_timeout, encoding="json")
            if sd_blob and 'blobs' in sd_blob:
                response['num_blobs_in_stream'] = len(sd_blob['blobs']) - 1
                head_blob_hash = sd_blob['blobs'][0]['blob_hash']
                head_blob_availability = yield self.jsonrpc_blob_availability(head_blob_hash,
                                                                              search_timeout,
                                                                              blob_timeout)
        ...
        response['sd_blob_availability'] = yield self.jsonrpc_blob_availability(sd_hash,
                                                                                search_timeout,
                                                                                blob_timeout)
